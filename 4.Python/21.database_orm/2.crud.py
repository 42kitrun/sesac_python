...
def get_user(session) -> list[User]:
    # 아무 인자도 안받아서, 사용자 리스트를 리턴하기
    return [ user for user in sess.query(User).all()]

def get_user_by_id(session, user_id:int) -> User | None:
    # 사용자 ID를 받아서 사용자 반환하기 (사용자 없을 수도 있음)
    # return next((user for user in sess.query(User).all() if user.id == user_id), None)
    # return sess.select(User).where(User.id == user_id)
...
def update_user_age(session, user_id:int, new_age:int) -> bool:
    ...
    # 그냥 객체에 값만 설정하려면?? 자동으로 쓰임 (물론 sess.commit()을 할 때..)
...
def delete_user_by_id(session, user_id:int)-> bool:
    ...
    # session.flush(user_to_delete) # session.flush = delete / session.delete = delete cascading
