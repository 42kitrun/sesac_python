...
print(my_list[1])
# print(my_list[5]) 
# 이게 JS에서는 undefined, 그 외 다른언어(python 포함)

print(my_list[4])
print(my_list[-1]) # 거꾸로 갈 수 있음. 파이썬의 특징. 대부분의 다른 언어는
...
my_list.pop(3) # 나의 리스트에서 인덱스3에 있는 요소를 삭제할거야 (숫자 4가 있음)
print(my_list)

my_list.pop() # 나의 리스트에서 인덱스3에 있는 요소를 삭제할거야 (숫자 4가 있음)
print(my_list)
...
print('--- 소팅전 ---')
print(my_list)
sort_list = sorted(my_list) # 일부 함수는, 복제본을 만들어서 반환하는 애가 있고, 
print('--- 소팅후 ---')
print (sort_list)

print('-'*10)
my_list.sort() # 일부 함수는, 복제본을 만들어서 반환하는 애가 있고, 원본 데이터를 고치는 애가 있다.
...
# 리스트 컴프리 헨션 # 파이선의 매~~우 큰 특징(장점)
numbers = [x for x in range(10)] # 리스트 안에 반복문 또는 조건문을 통해서, 리스트안에 채워질 요소를 정의할 수 있는 문법
print(numbers)

numbers = [mynumber for mynumber in range(5)] # 리스트 안에 반복문 또는 조건문을 통해서, 리스트안에 채워질 요소를 정의할 수 있는 문법
print(numbers)
