...
''' 아래 처럼 하나하나 안만들어도됨
@app.route('/user/honggd') # 사용자가 /user에 접속하면, 이 아래 함수를 호출해줘
def honggd():
    return "<h1>Hello, User!</h1>"
'''
...
if __name__ == '__main__':
    print('여기가 메인 함수')
    # app 시작! 사용자의 요청이 오는 것을 기다림
    app.run( port=5500, debug=True) # 절대로 이대로 커밋하면 안됨!! 꼭 끄고 커밋해야함.
    # debug=True 매번 서버를 껐다 켰다 할 필요없음
