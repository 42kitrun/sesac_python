...
# 상수는 이 위치에 대문자로 선언
UPLOAD_FOLDER = '6.Flask/2.templates/templates/uploads'
...
def get_file_size(file):
    ...
    file.stream.seek(0, os.SEEK_END) # file 안의 첫번째에서 마지막까지 포인터(fd)를 이동해
    size = file.stream.tell() # 너의 현재 위치를 알려줘
    file.stream.seek(pos) # 다시 처음 위치로 이동해라
    return size
...
def upload_file():
    ...
    # file_size = file.content_length #chrome 이 안 전달해줌
    ...
    if allowed_file_pythonic(file.filename):
        ...
        return redirect(url_for('index')) # 함수명 index()
    else:
