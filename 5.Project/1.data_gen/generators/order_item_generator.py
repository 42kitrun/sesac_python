    ...
    def load_id(self, _type, count:int):
        with open(f'output/{_type}.csv', 'r',encoding='utf-8') as csvfile:
            ...
            if len(csv_list) > count:
                return {'order':lambda:sample(csv_list[1:], k=count) # 처방은 전체 OrderId 중에서 count 만큼만(모두 1개씩 사는 최소 경우)
                       ,'item':lambda:csv_list[1:]}[_type]() # item은 전체 메뉴판(header 제거)에서 복원 추출
            else:
                return csv_list[1:] # 추출해야하는 수가 더 많으면 헤더를 제거한 전체 데이터셋 가져오기
    ...
    def generate( self, count):
        ...
        while len(order_items) <= count:
            ...
            order_grp = list(zip(
                                ...
                                ,[choice(self.id_map['order'])]* len(items_id_list) # order_id
...
if __name__ == '__main__': # 아래 스크립트는 본 파일을 직접 실행할 때만(module로 불러올때 말고)
    print(GenerateData(OrderItemGenerator(),10)())