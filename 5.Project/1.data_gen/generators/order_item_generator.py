import sys # 상위 폴더 추가
sys.path.append('/Users/seSAC/src/sesac_python/5.Project/1.data_gen')
from output import DisplayData
from id_generator import IdGenerator
from datetime import datetime
import csv
from find_id_info import find_id_info

class OrderItemGenerator:
    def __init__(self):
        self.id_gen = IdGenerator()

    def find_order_info(self, order_id) -> tuple:
    ...
    def find_item_info(self, item_id) -> tuple:
    ...
    def generate_order_items( self, order_id, item_list)->list:
        # 가장 먼저 유효한 거래인지 확인! 어떤 아이템들이 주문에서 결재되었는지
        order_info = self.id_gen.find_id_info('order',id = 'f8d2e618-ab11-439f-a321-b0593f7f70ec')
        ...
        DisplayData().save_csv('orderitem',order_items)
        # self.save_order_items(order_items) # csv 파일에 저장
        return order_items
...
if __name__ == '__main__': # 아래 스크립트는 본 파일을 직접 실행할 때만(module로 불러올때 말고)
    print(OrderItemGenerator().generate_order_items(order_id='f8d2e618-ab11-439f-a321-b0593f7f70ec'
