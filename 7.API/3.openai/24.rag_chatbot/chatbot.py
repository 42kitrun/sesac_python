...
from vectorstore import get_store # 다른 모듈의 변수를 바로 가져오지 말고, get_변수 로 가져오기
...
import json
def _load_prompts_from_json(filepath): # _fn : 내부함수
...
import yaml
def _load_prompts_from_yaml(filepath): # _fn : 내부함수
    with open(filepath, "r", encoding="utf-8") as f:
        data = yaml.safe_load(f) # yaml에 인젝션 공격을 방지하기 위해서 yaml 문법에 escaping 문자열로 나쁜짓 못하게..
