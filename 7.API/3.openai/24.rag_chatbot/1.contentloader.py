...
# Chroma는 개발자의 생산성과 "행복"에 초점을 맞춘 AI 네이티브 오픈 소스 벡터 데이터베이스
# Chroma is an AI-native open-source vector database.
# It comes with everything you need to get started built-in, and runs on your machine.
...
# Chroma는 AI 네이티브 오픈 소스 벡터 데이터베이스
# print(store)
...
# Retrieval Augmented Generation(RAG)에서 검색도구(Retrievers)는 벡터 저장소에서 문서를 검색하는 도구
...
# context는 retriever(검색도구)로부터 추출해서 {context} 라는 공간에 채워줄예정..
# 프롬프트 -> LLM -> 응답
